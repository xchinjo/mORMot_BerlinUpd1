Ext.define('ExtMVC.controller.Contacts', {
	...
	setFilter : function (btn) {
		...
		store.getProxy().extraParams = {
			where : 'REGNAT LIKE :("' + Ext.getCmp("Fname").value + '%"): AND REGDTE=:("' + Ext.Date.format(Ext.getCmp("Ftime").value, "Y-m-d") + '"):'
		};
		...
		var obj = item
			...
			store.filter([
					Ext.create('Ext.util.Filter', {property : "REGNAT", value : xname,root : 'values'}),
					Ext.create('Ext.util.Filter', {property : "REGDTE", value : xphone,root : 'values'}),
					Ext.create('Ext.util.Filter', {property : "REGCDE", value : xemail,root : 'values'})
				]);
	},
	...
	setFilterData : function (btn) {
		...
		store.remoteFilter = true;
		store.getProxy().extraParams = {
			where : 'REGDTE BETWEEN :("' + Ext.Date.format(Ext.getCmp("startdt").value, "Y-m-d") + '"): AND :("' + Ext.Date.format(Ext.getCmp("enddt").value, "Y-m-d") + '"):'
		};
		var obj = item
			for (var prop in obj) {
				var xphone = obj.timed;
